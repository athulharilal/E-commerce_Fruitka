            ...
            return new Promise(async(resolve, reject) => {
                db.get().collection(collection.ORDER_COLLECTION).aggregate([
                    ...
                    },
                    {
                        $match:{
                            date:{
                                $lt:new Date(),$gt: new Date(new Date().getTime()-(34*60*60*1000*365))
                            }
                        }
                    },
                    {
    ...
    catData:()=>{
        return new Promise((resolve, reject) => {
            db.get().collection(collection.ORDER_COLLECTION).aggregate([
                ...
                {
                  $unwind: "$products" // Split the array of products into individual documents
                },
                {
                  $lookup: {
                    ...
                    foreignField: "_id",
                    as: "product"
                  } // Lookup the product details based on the "item" field in the "products" array
                },
                {
                ...
                },
                {
                  $group: {
                    ...
                    total: { $sum: "$products.quantity" } // Calculate the total quantity sold per category
                  }
                },
                {
                  $lookup: {
                    ...
                    foreignField: "_id",
                    as: "category"
                  } // Lookup the category details based on the "_id" field
                },
                // {
                ...
                //     total: 1,
                //     _id: 0
                //   } // Project the category field and total quantity, excluding the "_id"
                // },
                // {
                //   $sort: { category: 1 } // Sort the results by category
                // }
              ]).toArray().then((catData)=>{
                resolve(catData)
              })
